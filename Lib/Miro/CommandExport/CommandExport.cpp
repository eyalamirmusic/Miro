...
std::string_view tsPrimitiveLocal(TypeTree::PrimitiveKind kind)
{
    using Kind = TypeTree::PrimitiveKind;
    switch (kind)
    {
        case Kind::Boolean:
            return "boolean";
...
const TypeTree::TypeNode*
    findRootByQualified(std::span<const TypeTree::TypeNode> roots,
...
const TypeTree::TypeNode::Field*
    findField(const TypeTree::TypeNode& node, std::string_view name)
{
...
struct KeyedInfo
{
    const TypeTree::TypeNode* itemType = nullptr;
    std::string_view keyTsType;
};

std::optional<KeyedInfo> resolveKeyedInfo(const TypeTree::TypeNode& payload,
                                          const EventEntry& event)
{
    ...
    if (collNode.shape != TypeTree::TypeNode::Shape::Array
        || collNode.inner == nullptr)
        return std::nullopt;

    auto& itemNode = *collNode.inner;
    if (itemNode.shape != TypeTree::TypeNode::Shape::Object)
        return std::nullopt;
    ...
    if (keyNode.shape != TypeTree::TypeNode::Shape::Primitive)
        return std::nullopt;
