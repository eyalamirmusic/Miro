...
auto xmlLoadPrimitives = test("XML: load primitives from attributes") = []
{
    ...
    auto s =
        createFromXMLString<ClassWithString>(R"(<ClassWithString name="world"/>)");
    check(s.name == "world");
};
...
{
    auto val =
        createFromXMLString<ClassWithObjectMap>("<ClassWithObjectMap><items>"
                                                "<alpha x=\"1\"/><beta x=\"2\"/>"
...
auto xmlSaveOptionalAbsent = test("XML: save optional absent omits attribute") = []
{
    ...
    // No assertion for maybeInner: atKey already materialized an empty child
    // element before writeNull ran, and load reads that back as absent.
};

auto xmlLoadOptionalAbsent = test("XML: load optional with missing attribute") = []
{
    auto val = createFromXMLString<ClassWithOptional>(R"(<ClassWithOptional/>)");
...
auto xmlSaveEnum = test("XML: save enum as attribute") = []
{
    ...
    // Only presence is asserted: the attribute is the enumerator name or its
    // underlying integer depending on which reflect path runs.
...
auto xmlRoundTripMaps = test("XML: round-trip maps") = []
{
    ...
    auto reloaded = createFromXMLString<ClassWithIntMap>(toXMLString(original));
...
auto xmlRoundTripIntegrals = test("XML: round-trip narrow integrals") = []
{
    ...
    auto reloaded = createFromXMLString<ClassWithIntegrals>(toXMLString(original));
...
auto xmlLoadIgnoresExtraAttributes =
    test("XML: load ignores unknown attributes") = []
{
